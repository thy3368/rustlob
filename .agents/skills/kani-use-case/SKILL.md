...
Use `kani` to prove business invariants of RustLOB use cases, not just arithmetic facts. Good proofs constrain symbolic inputs with meaningful assumptions, target `pre_check_command`, `validate_against_state`, or `compute_output_and_events`, and assert business properties that would matter if broken.
...
- For output invariants, call `compute_output_and_events` after proving command and state are admissible.
...
- Good example characteristics:
  ...
  - proof targets `CommandUseCase3` methods directly
