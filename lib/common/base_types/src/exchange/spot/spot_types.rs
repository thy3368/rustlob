...
/// 订单生命周期状态 - 描述订单从提交到终结的完整状态流转
///
/// ## 设计分析
///
/// **合理性**: ✅ 设计合理
/// - 7个状态覆盖订单完整生命周期（含条件单）
/// - 使用 `#[repr(u8)]` 优化内存占用（1字节）
...
/// | 状态 | 编码 | 类型 | 说明 |
/// |-----|------|------|------|
/// | Pending | 1 | 中间态 | 普通订单已接受，在订单簿等待成交 |
/// | PartiallyFilled | 2 | 中间态 | 部分数量已成交，剩余继续等待 |
/// | Filled | 3 | **终态** | 全部数量已成交，订单完成 |
/// | Cancelled | 4 | **终态** | 订单被取消（用户主动或系统） |
...
pub enum OrderStatus {
    /// **条件单待触发 (Conditional Pending)**
    ///
    /// 条件单（StopLoss/TakeProfit）已提交，正在等待触发条件满足。
    /// 此状态下订单未进入订单簿，不参与撮合。
    ///
    /// **特性**:
    /// - 初始状态：条件单提交后的第一个状态
    /// - 不可撮合：未进入订单簿，不参与撮合匹配
    /// - 可取消：用户可主动取消条件单
    /// - 自动触发：当价格满足触发条件时自动转为 Pending 状态
    ///
    /// **状态流转**:
    /// - ConditionalPending → Pending: 触发条件满足，订单激活进入订单簿
    /// - ConditionalPending → Cancelled: 用户主动取消条件单
    /// - ConditionalPending → Expired: GTD条件单到期（如设置过期时间）
    ///
    /// **资金状态**: 通常不冻结资金（或仅冻结部分保证金），触发后冻结
    ///
    /// **适用订单类型**:
    /// - StopLoss: 价格跌破止损价时触发
    /// - StopLossLimit: 价格跌破止损价时触发
    /// - TakeProfit: 价格上涨到止盈价时触发
    ...
    /// - Pending → Filled: 全部成交
    /// - Pending → Cancelled: 用户取消或系统风控取消
    /// - Pending → Expired: GTD订单到期
    ///
    /// **资金状态**: 已冻结所需资产
    Pending = 1,
    ...
    /// - PartiallyFilled → Cancelled: 取消未成交部分（转为IOC行为）
    ///
    /// **注意事项**:
    /// - 已成交部分不可取消
    /// - 部分成交的市价单可能因流动性不足而留下未成交部分
    PartiallyFilled = 2,
    ...
    /// **已过期 (Expired)**
    ///
    /// GTD (Good Till Date) 订单到达过期时间后自动取消。
    ///
    /// **特性**:
    /// - **终态**：状态不再改变
    /// - 仅适用于：TimeInForce = GTD 的订单
    /// - 自动触发：由系统定时任务处理
    ///
    /// **与 Cancelled 的区别**:
    /// - Cancelled: 主动取消（用户/系统）
    /// - Expired: 被动取消（时间到期）
    ///
    /// **资金状态**: 解冻全部冻结资金
    Expired = 6,
