...
use entity::{
    ...
    CommandWithGivenState, Entity, EntityFieldChange, EntityReplayableEvent, EventProjectError,
    HandlerLatencyMetrics, IssuedByParty, ObserveHandlerLatency, ReplayableChanges,
    UpdatedEntityPair,
};
...
impl ReplayableChanges for StubChanges {
    fn to_replayable_events(&self) -> Result<Vec<EntityReplayableEvent>, EventProjectError> {
        match self {
            Self::PlaceCreated { created_order } => Ok(vec![created_order.track_create_event()?]),
    ...
    fn compute_before_after_changes(
        ...
        match (cmd, state) {
            (StubCommand::Place { order_id, .. }, StubGivenState::Missing) => {
                ...
                Ok(StubChanges::PlaceCreated { created_order })
            }
            (StubCommand::Cancel { order_id, .. }, StubGivenState::Existing { order, .. }) => {
                ...
                Ok(StubChanges::Cancelled {
                    updated_order: UpdatedEntityPair { before: order.clone(), after },
                })
            }
...
fn command_and_state_branches_match_on_place_happy_path() {
    let use_case = StubUseCase6::default();
    let state = StubGivenState::Missing;
...
#[test]
fn create_branch_projects_events() {
    let use_case = StubUseCase6::default();
    let state = StubGivenState::Missing;
    let changes = use_case
        .compute_before_after_changes(
...
#[test]
fn update_branch_projects_events() {
    let use_case = StubUseCase6::default();
    let state = sample_existing_state(11);
    let changes = use_case
        .compute_before_after_changes(
            &StubCommand::Cancel { party_id: "trader-1".to_string(), order_id: 11 },
            &state,
...
#[test]
fn executor6_execute_continues_as_long_as_events_can_be_projected() {
    let executor = CommandUseCaseExecutor6;
    let use_case = StubUseCase6::default();
    let outbound = StubOutbound::with_state(StubGivenState::Missing);
    let observer = StubObserver::default();
