...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 模拟订单数据
    let buy_price = FixedPointArithmetic::from_f64(100.50, -2)?;
    let sell_price = FixedPointArithmetic::from_f64(100.55, -2)?;
    let quantity = FixedPointArithmetic::from_f64(1000.0, -2)?;
    ...
    // 计算中间价
    let two = FixedPointArithmetic::from_f64(2.0, -2)?;
    ...
    // 计算手续费 (0.1%)
    let fee_rate = FixedPointArithmetic::from_f64(0.001, -2)?;
    ...
    // 反序列化
    let received = FixedPointArithmetic::from_bytes(serialized);
    ...
    for i in 0..1000 {
        ...
        prices.push(FixedPointArithmetic::from_f64(price, -2)?);
    }
