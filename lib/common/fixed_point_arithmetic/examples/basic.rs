...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. 创建价格（股票精度：0.01）
    let price1 = FixedPointArithmetic::from_f64(123.45, -2)?;
    let price2 = FixedPointArithmetic::from_f64(56.78, -2)?;
    ...
    // 5. 除法
    let half = price1.checked_div(FixedPointArithmetic::from_f64(2.0, -2)?)?;
    ...
    // 7. 批量处理
    let prices = vec![
        FixedPointArithmetic::from_f64(100.0, -2)?,
        FixedPointArithmetic::from_f64(200.0, -2)?,
        FixedPointArithmetic::from_f64(300.0, -2)?,
    ];
    ...
    // 8. Unsafe极速版本（性能关键路径）
    unsafe {
        let fast_price = FixedPointArithmetic::from_f64_unchecked(999.99, -2);
        let fast_qty = FixedPointArithmetic::from_f64_unchecked(5.0, -2);
