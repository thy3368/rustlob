...
use super::{
    ...
    ObserveHandlerLatency, ReplayableChanges, UseCaseChanges, UseCaseReplyMapper,
};
use crate::HandlerLatencyMetrics;
use crate::command_use_case_def2::IssuedByParty;

#[derive(Debug, Clone, Copy, Default)]
pub struct CommandUseCaseExecutor5;

impl CommandUseCaseExecutor5 {
        ...
        U: CommandUseCase5,
    {
        if use_case.main_mi_name().is_empty() {
            return Err(EventProjectError::Custom(
                "CommandUseCase5 main_mi_name must not be empty".to_string(),
            ));
        }
        if use_case.main_mi_identity_field().is_empty() {
            return Err(EventProjectError::Custom(
                "CommandUseCase5 main_mi_identity_field must not be empty".to_string(),
            ));
        }
    ...
    /// 6. changes.to_replayable_events()
    /// 7. persist(events)
    /// 8. replay(events)
    /// 9. publish(events)
