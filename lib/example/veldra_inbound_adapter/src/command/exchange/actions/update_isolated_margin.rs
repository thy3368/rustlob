...
pub mod reply {
    pub use crate::command::exchange::common::wire::ExchangeEmptyResponseWire as UpdateIsolatedMarginResponseWire;
}
...
fn validate(request: &RequestWire) -> Result<(), ExchangeHttpError> {
    if request.action.type_ != "updateIsolatedMargin" {
        return Err(ExchangeHttpError::contract(
            UpdateIsolatedMarginContractError::UnexpectedActionType(request.action.type_.clone()),
        ));
    }
