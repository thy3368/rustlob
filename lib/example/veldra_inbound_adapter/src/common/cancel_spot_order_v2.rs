...
mod tests {
    use super::*;
    use example_core_use_case::{
        CancelSpotOrderV2CmdV3, CancelSpotOrderV2LookupV3, SpotOrderV2CommandV3, SpotOrderStatus,
    };
    ...
    fn execute_cancel_open_order_releases_reservation_and_projects_events() {
        ...
        let updated_events = result.events.iter().filter(|event| event.is_updated()).collect::<Vec<_>>();
        assert!(!updated_events.is_empty());
        assert!(updated_events
            .iter()
            .all(|event| event.new_version == event.old_version + 1 && !event.field_changes.is_empty()));
    }
}
