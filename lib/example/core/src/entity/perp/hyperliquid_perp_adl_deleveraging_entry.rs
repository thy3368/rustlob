...
const HYPERLIQUID_PERP_ADL_DELEVERAGING_ENTRY_ENTITY_TYPE: u8 = 19;

/// 单个对手方仓位被 ADL 去杠杆的一条 append-only 事实。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HyperliquidPerpAdlDeleveragingEntry {
    pub adl_deleveraging_entry_id: String,
...
impl HyperliquidPerpAdlDeleveragingEntry {
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        adl_deleveraging_entry_id: String,
        adl_batch_id: String,
...
impl Entity for HyperliquidPerpAdlDeleveragingEntry {
    type Id = String;

    fn entity_id(&self) -> Self::Id {
        self.adl_deleveraging_entry_id.clone()
    }

    fn entity_type() -> u8 {
        HYPERLIQUID_PERP_ADL_DELEVERAGING_ENTRY_ENTITY_TYPE
    }
    ...
    fn replay_entity_id(&self) -> Result<i64, EntityError> {
        Ok(stable_entity_id(&self.adl_deleveraging_entry_id))
    }
}
...
mod tests {
    ...
    #[test]
    fn create_event_contains_adl_entry_fields() {
        let entry = HyperliquidPerpAdlDeleveragingEntry::new(
            "adl-entry-1".to_string(),
        ...
        assert!(event.field_changes.iter().any(|change| {
            change.field_name_as_str().ok() == Some("adl_deleveraging_entry_id")
                && change.new_value_bytes() == b"adl-entry-1"
        }));
    }
}
