...
impl QueryUseCase for QueryHyperliquidPerpLiquidationCandidatesUseCase {
    ...
    fn compute_view(
        ...
        for snapshot in read_model.snapshots {
            ...
            if !snapshot
                .position
                .liquidation_triggered_by_mark_price(snapshot.mark_price, snapshot.bankruptcy_price)
            {
                continue;
            }

            let reason = trigger_reason(&snapshot);
...
fn trigger_reason(
    snapshot: &HyperliquidPerpRiskSnapshot,
) -> HyperliquidPerpLiquidationTriggerReason {
    if snapshot.available_margin == 0 {
