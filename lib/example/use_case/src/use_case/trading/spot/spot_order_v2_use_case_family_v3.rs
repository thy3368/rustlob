...
impl ReplayableChanges for PlaceTriggerPendingSpotOrderV2ChangesV3 {
    fn to_replayable_events(
        ...
        Ok(Vec::from([self.created_order.track_create_event()?]))
    }
}
...
impl ReplayableChanges for TriggerSpotOrderV2ChangesV3 {
    fn to_replayable_events(
        ...
        let updated_order_event_capacity =
            if self.updated_order.after != self.triggered_order { 2 } else { 1 };
        let event_capacity = self
            .created_trades
            .len()
            .saturating_add(self.updated_maker_orders.len())
            .saturating_add(updated_order_event_capacity)
            .saturating_add(self.created_vouchers.len())
            .saturating_add(self.created_balance_ledger_entries.len())
            .saturating_add(self.created_balance_ledger_entries.len());
        let mut events = Vec::with_capacity(event_capacity);
...
impl StateMachineV2Unchecked for SpotOrderV2UseCaseFamilyV3 {
    type Command = SpotOrderV2CommandV3;
    type StateGiven = SpotOrderV2GivenStateV3;
    type Error = SpotOrderV2UseCaseFamilyV3Error;
    type StateChanged = SpotOrderV2AfterChangesV3;
...
mod tests {
    ...
    fn place_gtc_without_cross_keeps_state_and_outputs_no_side_effects() {
        ...
        let SpotOrderV2AfterChangesV3::Place(after) =
            family.compute_state_changed(&place_cmd("gtc"), &state).unwrap()
        else {
    ...
    fn place_ioc_partial_fill_releases_remainder() {
        ...
        let SpotOrderV2CaseChangesV3::Place(changes) =
            family.compute_state_diff(&place_cmd("ioc"), state).unwrap()
        else {
    ...
    fn merge_before_after_extracts_before_truth_from_given_state() {
        ...
        let after = family.compute_state_changed(&place_cmd("ioc"), &state).unwrap();

        let SpotOrderV2CaseChangesV3::Place(changes) =
            SpotOrderV2UseCaseFamilyV3::do_compute_state_diff(state, after).unwrap()
        else {
            panic!("expected place case changes");
        };
    ...
    fn place_alo_cross_rejects_and_releases() {
        ...
        let SpotOrderV2AfterChangesV3::Place(after) =
            family.compute_state_changed(&place_cmd("alo"), &state).unwrap()
        else {
