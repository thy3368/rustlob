...
use super::types::{
    PositionId, TraderId, Price, Quantity, PositionSide, MarginMode, Leverage, Margin, Timestamp,
};
...
impl Position {
    ...
    pub fn new(
        ...
        let liquidation_price = Self::calc_liquidation_price(
            entry_price, position_side, leverage, margin_mode,
        );
    ...
    fn update_liquidation_price(&mut self) {
        self.liquidation_price = Self::calc_liquidation_price(
            self.entry_price, self.position_side, self.leverage, self.margin_mode,
        );
    }
