...
impl IdRepo {
    pub(crate) fn next_event_id(&mut self) -> i64 {
        EVENT_ID_GEN.next_id()
    }

    pub(crate) fn next_transaction_id(&mut self) -> i64 {
        TRANS_ID_GEN.next_id()
    }

    pub(crate) fn next_order_id(&mut self) -> i64 {
        ORDER_ID_GEN.next_id()
    }

    pub(crate) fn next_trade_id(&mut self) -> i64 {
        TRADE_ID_GEN.next_id()
    }

    // 兼容 u64 的方法（用于事件系统）
    pub(crate) fn next_event_id_u64(&mut self) -> u64 {
        self.next_event_id() as u64
    }

    pub(crate) fn next_transaction_id_u64(&mut self) -> u64 {
