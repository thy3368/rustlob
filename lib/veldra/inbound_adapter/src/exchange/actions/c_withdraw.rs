...
pub enum CWithdrawContractError {
    #[error("Unexpected `action.type` for cWithdraw handler: `{0}`.")]
    ...
    NonceMismatch,
    #[error("`vaultAddress` is not supported for `cWithdraw`.")]
    VaultAddressNotSupported,
    #[error("`expiresAfter` is not supported for `cWithdraw`.")]
    ExpiresAfterNotSupported,
}

pub mod reply {
    pub use crate::exchange::common::wire::ExchangeEmptyResponseWire as CWithdrawResponseWire;
}
...
impl ExchangeActionHandler for CWithdrawAction {
    type Request = RequestWire;
    type Reply = reply::CWithdrawResponseWire;
...
pub async fn handle(body: &[u8]) -> Result<reply::CWithdrawResponseWire, ExchangeHttpError> {
    run_exchange_action::<CWithdrawAction>(body).await
}
