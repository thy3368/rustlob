...
pub enum UserDexAbstractionContractError {
    #[error("Unexpected `action.type` for userDexAbstraction handler: `{0}`.")]
    ...
    NonceMismatch,
    #[error("`expiresAfter` is not supported for `userDexAbstraction`.")]
    ExpiresAfterNotSupported,
}

pub mod reply {
    pub use crate::exchange::common::wire::ExchangeEmptyResponseWire as UserDexAbstractionResponseWire;
}
...
struct UserDexAbstractionAction;

impl ExchangeActionHandler for UserDexAbstractionAction {
    type Request = RequestWire;
    type Reply = reply::UserDexAbstractionResponseWire;
...
) -> Result<reply::UserDexAbstractionResponseWire, ExchangeHttpError> {
    run_exchange_action::<UserDexAbstractionAction>(body, deps).await
}
