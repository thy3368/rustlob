...
fn spot_order_commitment(order: &SpotOrderV2) -> String {
    ...
    stable_hash_hex(&[
        ...
        order.status.as_str(),
        status_reason,
        order.reservation.reservation_id.as_str(),
        order.reservation.reservation_kind.as_str(),
        order.reservation.asset_id.as_str(),
        order.reservation.original_amount.to_string().as_str(),
        order.reservation.remaining_amount.to_string().as_str(),
        order.fee_reservation.reservation_id.as_str(),
        order.fee_reservation.reservation_kind.as_str(),
        order.fee_reservation.asset_id.as_str(),
        order.fee_reservation.original_amount.to_string().as_str(),
        order.fee_reservation.remaining_amount.to_string().as_str(),
