...
HyperCore state includes an order book for each asset. The order book functions similarly to centralized exchanges. Orders are placed where price is an integer multiple of the tick size, and size is an integer multiple of lot size. Orders are matched in price-time priority.
