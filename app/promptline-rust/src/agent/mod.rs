...
use crate::agent::agent_int::AgentInt;
...
impl AgentInt for Agent {
    /// Run the agent on a task
    async fn run(&mut self, task: &str) -> Result<AgentResult> {
...
mod tests {
    ...
    async fn test_agent_simple_task() {
        ...
        let permission_manager = std::sync::Arc::new(std::sync::Mutex::new(
            crate::permissions::PermissionManager::new().unwrap(),
        ));
        permission_manager
            .lock()
            .unwrap()
            ...
            .unwrap();
        let mut agent = Agent::new(model, tools, config, Vec::new(), permission_manager)
            .await
            .unwrap();
