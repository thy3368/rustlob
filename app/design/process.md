...
```
外部角色 → API调用 → 触发任务(Command) → 系统执行业务逻辑 → 生成EntityChangeLog → 驱动实体状态变化
```
...
| 任务(Command) | 说明 | 触发角色 | API端点 | 请求参数 | 生成的EntityChangeLog | 驱动的实体状态变化 |
|--------------|------|---------|---------|---------|---------------------|------------------|
...
**EntityChangeLog示例 - SubmitLimitOrder**:
```json
{
  "logId": "log_20250104_001",
  "aggregateType": "Order",
...
```json
{
  "logId": "log_20250104_002",
  "aggregateType": "Balance",
  "aggregateId": "bal_user12345_usdt",
  "eventType": "BalanceFrozenEvent",
  "timestamp": "2025-01-04T10:30:15.125Z",
  "actor": {
    "roleType": "System",
...
**EntityChangeLog示例 - ApproveWithdrawal**:
```json
{
  "logId": "log_20250104_100",
  "aggregateType": "Withdrawal",
  "aggregateId": "wd_20250104_001",
  "eventType": "WithdrawalApprovedEvent",
  "timestamp": "2025-01-04T11:20:30.000Z",
  "actor": {
    "roleType": "ComplianceOfficer",
    "userId": "officer_alice",
    "userName": "alice@exchange.com"
  },
  "changeData": {
    "approvalNotes": "Identity verified, legitimate transaction",
    "riskScore": 35,
    "reviewDurationSeconds": 1800
  },
  "previousState": {
    "withdrawalId": "wd_20250104_001",
  ...
  "newState": {
    ...
    "address": "bc1q...",
    "network": "BTC",
    "approvedBy": "officer_alice",
    "approvedAt": "2025-01-04T11:20:30.000Z"
  },
  "metadata": {
    "correlationId": "req_withdrawal_001",
    "causationId": "log_20250104_099",
    "sequenceNumber": 5,
    "ipAddress": "10.0.0.50"
  }
}
...
**EntityChangeLog示例 - ApproveKYC**:
```json
{
  "logId": "log_20250104_200",
  "aggregateType": "KYCApplication",
  "aggregateId": "kyc_user12345",
  "eventType": "KYCApprovedEvent",
  "timestamp": "2025-01-04T14:30:00.000Z",
  "actor": {
    "roleType": "ComplianceOfficer",
    "userId": "officer_bob",
    "userName": "bob@exchange.com"
  },
  "changeData": {
    "verificationLevel": "L2",
    "verifiedFields": ["name", "birthDate", "nationality", "idNumber", "address"],
    "reviewNotes": "All documents verified successfully"
  },
  "previousState": {
    "applicationId": "kyc_user12345",
    "userId": "user_12345",
    "status": "UNDER_REVIEW",
    "currentLevel": "L0"
  },
  "newState": {
    ...
    "approvedAt": "2025-01-04T14:30:00.000Z"
  },
  "metadata": {
    "correlationId": "req_kyc_001",
    "causationId": "log_20250104_199",
    "sequenceNumber": 8
  }
}
```
...
## EntityChangeLog 标准格式

### 完整字段定义

```typescript
interface EntityChangeLog {
  // 事件标识
  logId: string;              // 唯一日志ID
  aggregateType: string;      // 聚合根类型: Order, Balance, Position, etc.
  aggregateId: string;        // 聚合根实例ID
  eventType: string;          // 事件类型: OrderSubmittedEvent, etc.
  eventVersion: string;       // 事件版本: "1.0"
  timestamp: string;          // ISO8601时间戳

  // 触发者信息
  actor: {
    roleType: string;         // Trader, Admin, ComplianceOfficer, System
    userId?: string;          // 用户ID（如果是用户触发）
    userName?: string;        // 用户名（可选）
    apiKeyId?: string;        // API密钥ID（如果是API调用）
    systemComponent?: string; // 系统组件名（如果是系统内部触发）
  };

  // 变更数据
  changeData: object;         // 事件特定的变更数据

  // 状态快照
  previousState?: object;     // 变更前状态（可选）
  newState: object;           // 变更后状态

  // 元数据
  metadata: {
    correlationId: string;    // 请求关联ID（追踪用户请求）
    causationId?: string;     // 因果关系ID（指向触发此事件的父事件）
    sequenceNumber: number;   // 该聚合根的事件序列号
