...
use dex::cmd_handler::{
    ExchangeCommand, ExchangeCommandEnvelope, PerpAmendOrderCmd, PerpCommand, PerpPlaceOrderCmd,
    PerpSide, ProductType, SubmitTradingCommandHandler, TradingCommand,
};

#[test]
fn perp_amend_order_command_can_enter_pending_queue() {
    ...
    let cmd = ExchangeCommandEnvelope {
        ...
        timestamp_ns: 20_001,
        product_type: ProductType::Perp,
        command: ExchangeCommand::TradingCommand(TradingCommand::Perp(PerpCommand::AmendOrder(
            PerpAmendOrderCmd {
                trader_id: 7,
                order_id: 200,
