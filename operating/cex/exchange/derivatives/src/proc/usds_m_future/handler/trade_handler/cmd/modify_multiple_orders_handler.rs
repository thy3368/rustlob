...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ModifyMultipleOrdersCmd, UsdsMFutureTradeCmdError,
};

pub struct ModifyMultipleOrdersCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ModifyMultipleOrdersCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ModifyMultipleOrdersCmdHandler<R, P> {
    type Command = ModifyMultipleOrdersCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ModifyMultipleOrdersCmdHandler<R, P> {}
