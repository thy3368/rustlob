...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    CancelAllOpenOrdersCmd, UsdsMFutureTradeCmdError,
};

pub struct CancelAllOpenOrdersCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> CancelAllOpenOrdersCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for CancelAllOpenOrdersCmdHandler<R, P> {
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for CancelAllOpenOrdersCmdHandler<R, P> {}
