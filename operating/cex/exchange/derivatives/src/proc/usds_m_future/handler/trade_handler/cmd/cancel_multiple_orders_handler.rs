...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    CancelMultipleOrdersCmd, UsdsMFutureTradeCmdError,
};

pub struct CancelMultipleOrdersCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> CancelMultipleOrdersCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for CancelMultipleOrdersCmdHandler<R, P> {
    type Command = CancelMultipleOrdersCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for CancelMultipleOrdersCmdHandler<R, P> {}
