...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ChangeInitialLeverageCmd, UsdsMFutureTradeCmdError,
};

pub struct ChangeInitialLeverageCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ChangeInitialLeverageCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ChangeInitialLeverageCmdHandler<R, P> {
    type Command = ChangeInitialLeverageCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ChangeInitialLeverageCmdHandler<R, P> {}
