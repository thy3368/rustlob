...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    NewOrderTestCmd, UsdsMFutureTradeCmdError,
};

pub struct NewOrderTestCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> NewOrderTestCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for NewOrderTestCmdHandler<R, P> {
    type Command = NewOrderTestCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for NewOrderTestCmdHandler<R, P> {}
