...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ChangeMultiAssetsModeCmd, UsdsMFutureTradeCmdError,
};

pub struct ChangeMultiAssetsModeCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ChangeMultiAssetsModeCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ChangeMultiAssetsModeCmdHandler<R, P> {
    type Command = ChangeMultiAssetsModeCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ChangeMultiAssetsModeCmdHandler<R, P> {}
