...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ModifyOrderCmd, UsdsMFutureTradeCmdError,
};

pub struct ModifyOrderCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ModifyOrderCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ModifyOrderCmdHandler<R, P> {
    type Command = ModifyOrderCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ModifyOrderCmdHandler<R, P> {}
