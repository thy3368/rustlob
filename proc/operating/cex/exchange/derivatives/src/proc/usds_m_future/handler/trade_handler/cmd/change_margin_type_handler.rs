...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ChangeMarginTypeCmd, UsdsMFutureTradeCmdError,
};

pub struct ChangeMarginTypeCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ChangeMarginTypeCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ChangeMarginTypeCmdHandler<R, P> {
    type Command = ChangeMarginTypeCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ChangeMarginTypeCmdHandler<R, P> {}
