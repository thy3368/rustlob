...
use crate::proc::usds_m_future::behavior::trade_behavior::{
    ChangePositionModeCmd, UsdsMFutureTradeCmdError,
};

pub struct ChangePositionModeCmdHandler<R: CmdRepo2, P: EventPublisher2> {
    pub repo: R,
    pub publisher: P,
}

impl<R: CmdRepo2, P: EventPublisher2> ChangePositionModeCmdHandler<R, P> {
    pub fn new(repo: R, publisher: P) -> Self {
        Self { repo, publisher }
    }
}

impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerInternal for ChangePositionModeCmdHandler<R, P> {
    type Command = ChangePositionModeCmd;
    type Reply = ();
    type GivenStateSet = ();
...
impl<R: CmdRepo2, P: EventPublisher2> CmdHandlerForUpdate3 for ChangePositionModeCmdHandler<R, P> {}
