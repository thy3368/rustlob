...
use crate::proc::{
    ...
    repo::{
        symbol_lob_repo::{MultiSymbolLobRepo, SymbolLobRepo},
        local_lob_repo_impl::LocalLob
    }
};
...
#[allow(dead_code)]
pub struct StandaloneLobRepo {
    lobs: HashMap<Symbol, LocalLob>
}

impl StandaloneLobRepo {
    /// 创建新的 SingleLobRepo
    ///
    /// # 参数
    /// - `lobs`: LOB 的向量，将被转换为 HashMap
    #[allow(dead_code)]
    pub fn new(lobs: Vec<LocalLob>) -> Self {
    ...
    /// 从 HashMap 直接创建
    #[allow(dead_code)]
    pub fn from_map(lobs: HashMap<Symbol, LocalLob>) -> Self {
        Self {
            lobs
        }
    }
...
impl MultiSymbolLobRepo for StandaloneLobRepo {
    fn match_orders(
        &self, symbol: Symbol, side: Side, price: Price, quantity: Quantity
    ) -> Option<Vec<&InternalOrder>> {
        // O(1) 查找对应的 LOB
...
mod tests {
    ...
    fn test_match_orders_symbol_found() {
        ...
        let lob1 = LocalLob::new(btc_symbol);
        let lob2 = LocalLob::new(eth_symbol);
    ...
    fn test_match_orders_symbol_not_found() {
        // 创建只有 BTCUSDT 的 repo
        let btc_symbol = Symbol::new("BTCUSDT");
        let lob = LocalLob::new(btc_symbol);
    ...
    fn test_empty_repo() {
        // 创建空的 repo
        let repo = StandaloneLobRepo::new(vec![]);
    ...
    fn test_multiple_lobs_with_same_symbol() {
        ...
        let btc_symbol = Symbol::new("BTCUSDT");
        let lob1 = LocalLob::new(btc_symbol);
        let lob2 = LocalLob::new(btc_symbol);
    ...
    fn test_from_map_constructor() {
        ...
        map.insert(btc_symbol, LocalLob::new(btc_symbol));
        map.insert(eth_symbol, LocalLob::new(eth_symbol));
    ...
    fn test_hashmap_performance() {
        ...
        let lobs: Vec<LocalLob> = symbols.iter().map(|s| LocalLob::new(*s)).collect();
    ...
    fn test_multi_lob_repo_trait_contains_symbol() {
        ...
        // 使用 trait 方法
        let repo_trait: &dyn MultiSymbolLobRepo = &repo;
    ...
    fn test_polymorphism_with_trait_object() {
        ...
        let repo_trait: Box<dyn MultiSymbolLobRepo> = Box::new(repo);
